...
def get_seeds(
    nseeds, boxsize=None, selection_function='uniform',
    sampling_data=None
):
    if selection_function == 'randoms':
        ...
        seeds = np.random.rand(nseeds, 3) * boxsize
    return seeds
...
def get_density_pdf(
    ...
    randoms_positions2=None, randoms_weights2=None, boxsize=None,
    ...
    if selection_function == 'randoms':
        ...
        if smooth_type == 'gaussian':
            D1D2 = jl.eval("count_pairs_box_gaussian(positions1, positions2, weights1, weights2, boxsize, smooth_radius)")
        elif smooth_type == 'tophat':
            D1D2 = jl.eval("count_pairs_box(positions1, positions2, weights1, weights2, boxsize, smooth_radius)")
        else:
            raise ValueError("Smoothing filter must be 'tophat' or 'gaussian'.")

        bin_volume = 4/3 * np.pi * smooth_radius ** 3
        mean_density = np.sum(data_weights2) / (boxsize[0]*boxsize[1]*boxsize[2])
